...
class DownloadMediaJob implements ShouldQueue
{
    ...
    public function handle(TelegramService $telegramService, YtDlpService $ytDlpService): void
    {
        ...
        try {
            ...
            // Send images as documents (to preserve original quality, no resize/compress)
            if (!empty($images)) {
                Log::info('Sending images to user as documents (no resize)', [
                    'chat_id' => $this->chatId,
                    'images_count' => count($images),
                    'image_paths' => array_map('basename', $images),
                ]);
                
                // Send each image as document individually (preserves original quality)
                // sendDocument doesn't support media groups, so we send one by one
                $successCount = 0;
                foreach ($images as $index => $imagePath) {
                    $docCaption = ($index === 0) ? $caption : null;
                ...
                if ($successCount === 0) {
                    Log::error('Failed to send all images as documents', [
                        'chat_id' => $this->chatId,
                        'images_count' => count($images),
                    ]);
                    ...
                    Log::info('Images sent as documents successfully', [
                        'chat_id' => $this->chatId,
                        'sent' => $successCount,
                        'total' => count($images),
                    ]);
